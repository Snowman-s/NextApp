...
export default function celebrate(p: CustomP5) {
  ...
  function initialize() {
    backgroundRenderers = [
      backgroundRendererCandidates[
        p.int(p.random(backgroundRendererCandidates.length))
      ](),
    ];
    stringRenderers = [
      stringRendererCandidates[
        p.int(p.random(stringRendererCandidates.length))
      ](),
    ];

    effectRenderers = RandomChoice.choice(
      effectRendererCandidates,
      p.int(happiness / 50) + 1,
      (max) => p.int(p.random(max))
    ).map((t) => t());
